...
# -----------------------------------------------------------------------------------------------------------------
#  Check each login - remote login
# -----------------------------------------------------------------------------------------------------------------

function check_user {

	# curl -H 'Accept: application/json' "http://localhost:7001/api/v2/login?username=$1@gmail.com&password=abcdefghi123" \
	#		-H 'Accept: application/json' "https://easy-2c-why.com/api/v2/login?username=$1&password=$3" \
	#	-H 'Accept: application/json' "http://127.0.0.1:7001/api/v2/login?username=$1&password=$3" \

	curl  \
		-H 'Accept: application/json' "https://easy-2c-why.com/api/v2/login?username=$1&password=$3" \
		-o out/ct_login_acct.$1.out
	cat out/ct_login_acct.$1.out
	echo
	if grep "status.*success" out/ct_login_acct.$1.out ; then
		:
	else
...
check_user "abando@uwyo.edu" "Bando, Arsene" pw1858693sa50b "x" sa50
check_user "cdepaol1@uwyo.edu" "Depaolo, Cale J." pw1858959sa51b "x" sa51
check_user "myraiken@uwyo.edu" "Hockenberry, James W" pw539040sa52b "x" sa52
check_user "pschlump1@uwyo.edu" "Schlump, Test A" pw119010sa53b "x" sa53
check_user "pschlump2@uwyo.edu" "Schlump, Test B" pw129010sa54b "x" sa54
