...
function submitForm35 ( event ) {
    event.preventDefault(); // Totally stop stuff happening

	console.log ( "Click of submit button for form35 - login" );
...
//				{ReqVar: "note", ParamName: "p_note"},
	...
	submitItData ( event, data, "/api/v1/h_evShipEnd", function(data){
...
function renderForm35 ( event ) {
	var form = [ ''
		,'<div>'
			,'<div class="row">'
				,'<div class="col-sm-6">'
					,'<div class="card bg-default">'
						,'<div class="card-header"><h2>Shiping Receivied</h2></div>'
						,'<div class="card-body">'
							,'<form id="form01">'
								...
								,'</div>'
								,'<button type="button" class="btn btn-primary" id="form35-submit">Received</button>'
							,'</form>'
						,'</div>'
					,'</div>'
				,'</div>'
	...
	$("#body").html(form);
	// Add events
	$("#form35-submit").click(submitForm35);
	// xyzzy - additional click events forgot-pass, forgot-acct
}
$("#evShipEnd").click(renderForm35); 	// Attach to link to paint the partial
