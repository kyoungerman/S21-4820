...
function submitForm36 ( event ) {
    event.preventDefault(); // Totally stop stuff happening

	console.log ( "Click of submit button for form36 - login" );
...
//				{ReqVar: "note", ParamName: "p_note"},
	...
	submitItData ( event, data, "/api/v1/h_evInspection", function(data){
...
function renderForm36 ( event ) {
	var form = [ ''
		,'<div>'
			,'<div class="row">'
				,'<div class="col-sm-6">'
					,'<div class="card bg-default">'
						,'<div class="card-header"><h2>Product Inspection</h2></div>'
						,'<div class="card-body">'
							,'<form id="form01">'
								...
								,'</div>'
								,'<button type="button" class="btn btn-primary" id="form36-submit">Inspected</button>'
							,'</form>'
						,'</div>'
					,'</div>'
				,'</div>'
	...
	$("#body").html(form);
	// Add events
	$("#form36-submit").click(submitForm36);
	// xyzzy - additional click events forgot-pass, forgot-acct
}
$("#evInspection").click(renderForm36); 	// Attach to link to paint the partial
