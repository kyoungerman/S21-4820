...
CREATE TABLE ct_login (
	  user_id					char varying(40) not null primary key -- 1 to 1 to t_ymux_user."id"
...
CREATE OR REPLACE function ct_lesson_seen_upd()
RETURNS trigger AS 
$BODY$
BEGIN
...
CREATE TRIGGER ct_lesson_seen_trig
BEFORE update ON "ct_lesson_seen"
FOR EACH ROW
EXECUTE PROCEDURE ct_lesson_seen_upd();
...
CREATE OR REPLACE function ct_lesson_upd()
RETURNS trigger AS 
$BODY$
BEGIN
	NEW.updated := current_timestamp;
...
CREATE TRIGGER ct_lesson_trig
BEFORE update ON "ct_lesson"
FOR EACH ROW
EXECUTE PROCEDURE ct_lesson_upd();
...
CREATE OR REPLACE function ct_lesson_validation_upd()
RETURNS trigger AS 
$BODY$
BEGIN
	NEW.updated := current_timestamp;
...
CREATE TRIGGER ct_lesson_validation_trig
BEFORE update ON "ct_lesson_validation"
FOR EACH ROW
EXECUTE PROCEDURE ct_lesson_validation_upd();
